...
module podium::PodiumPass_test {
    ...
    fun test_buy_fee_distribution(
        ...
        assert!(coin::balance<AptosCoin>(@podium) == initial_treasury_balance + base_price + protocol_fee, 1);
    ...
    fun test_sell_fee_distribution(
        ...
        // Verify fee distribution
        assert!(coin::balance<AptosCoin>(@podium) == initial_treasury_balance - sell_price, 1);
        assert!(coin::balance<AptosCoin>(signer::address_of(target)) == initial_target_balance + subject_fee, 2);
    ...
    fun test_buy_sell_bonding_curve_funds(
        ...
        let mid_contract_balance = coin::balance<AptosCoin>(@podium);
        ...
        let final_contract_balance = coin::balance<AptosCoin>(@podium);
